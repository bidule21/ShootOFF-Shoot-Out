...
public class ShootOut extends ProjectorTrainingExerciseBase implements TrainingExercise {
	...
	private final List<File> acceptedTargets = Arrays.asList(new File("targets/AQT_Silhouette.target"),
			new File("targets/IPSC.target"), new File("targets/USPSA.target"));
	...
	private void startRound(String coursePath) {
		...
		for (int i = 0; i < targetCount; i++) {
			...
			case NEAR:
				scale = (double) (rand.nextInt((100 - 60) + 1) + 80) / 100d;
				break;

			case MEDIUM:
				scale = (double) (rand.nextInt((70 - 50) + 1) + 50) / 100d;
				break;

			case FAR:
				scale = (double) (rand.nextInt((40 - 20) + 1) + 20) / 100d;
				break;
